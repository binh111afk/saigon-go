...
async function fixRinRinPark() {
  console.log('🚀 Bắt đầu cào ĐÚNG CHUẨN ảnh Công viên cá Koi Rin Rin Park...');

  if (!fs.existsSync(SPOT_FILE)) return;
  let spots = JSON.parse(fs.readFileSync(SPOT_FILE, 'utf8'));

  const rinrinIndex = spots.findIndex((s) => s.id === 2 || s.name.includes('Rin Rin'));
  if (rinrinIndex === -1) {
    console.error('❌ Không tìm thấy Rin Rin Park trong stroll.json');
    return;
  }
  ...
  try {
    const searchQuery = encodeURIComponent('Công Viên Đá Nhật - RinRin Park Hóc Môn');
    console.log(`  🔎 Tim kiếm Google Maps: "Công Viên Đá Nhật - RinRin Park Hóc Môn"...`);
    ...
    if (page.url().includes('/maps/place/')) {
      spots[rinrinIndex].mapUrl = page.url();
      console.log(`  🌐 Google Maps URL: ${page.url()}`);
    }
    ...
    console.log(`  📸 Đã thu thập được ${finalPhotos.length} ảnh CHUẨN RIN RIN PARK.`);

    if (finalPhotos.length > 0) {
      const uploadedUrls = [];
      const folderPath = 'địa điểm/đi dạo/Rin Rin Park';
      console.log(`  ☁️ Upload 8 ảnh chuẩn lên Cloudinary [${folderPath}]...`);
      ...
      if (uploadedUrls.length > 0) {
        spots[rinrinIndex].images = uploadedUrls;
      }
    }
  } catch (err) {
    console.error(`  ❌ Lỗi cào ảnh Rin Rin Park:`, err.message);
  } finally {
    await page.close();
  }
  ...
  console.log(`  💾 Đã lưu bộ ảnh mới chuẩn Rin Rin Park vào stroll.json!`);
}

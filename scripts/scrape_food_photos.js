...
const SPOT_FILE = path.join(process.cwd(), 'src', 'data', 'spots', 'food.json');
...
async function runFoodScraper() {
  console.log('🚀 Bắt đầu cào ĐỦ ẢNH GOOGLE MAPS CHO FOOD.JSON (20 QUÁN ẨM THỰC)...');
  ...
  for (let i = 0; i < spots.length; i++) {
    ...
    const folderPath = `địa điểm/ẩm thực/${cleanName}`;
  ...
  await browser.close();
  console.log(`\n🎉 HOÀN THÀNH TOÀN BỘ CÀO ẢNH CHO FOOD.JSON!`);
}

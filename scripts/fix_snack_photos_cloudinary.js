...
const SPOT_FILE = path.join(process.cwd(), 'src', 'data', 'spots', 'snack.json');
...
async function fixSnackPhotos() {
  console.log('🚀 Bắt đầu cào & Upload ảnh MỚI CHUẨN 200 OK cho snack.json...');
  ...
  for (let i = 0; i < spots.length; i++) {
    const venue = spots[i];
    console.log(`\n--------------------------------------------------`);
    console.log(`[${i + 1}/${spots.length}] Xử lý: ${venue.name}`);
    ...
    try {
      ...
      console.log(`  📸 Tìm thấy ${finalPhotos.length} ảnh hợp lệ cho [${venue.name}].`);
    ...
    console.log(`  💾 Đã lưu ${venue.images ? venue.images.length : 0} ảnh Cloudinary chuẩn cho [${venue.name}]!`);

    await randomDelay(1000, 2000);
  }

  await browser.close();
  console.log(`\n🎉 HOÀN THÀNH FIX ẢNH SNACK.JSON!`);
}
